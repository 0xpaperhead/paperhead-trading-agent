...
export async function depositToAgent(amount: number): Promise<{
  success: boolean
  message: string
  error?: string
}> {
  try {
    ...
    if (success) {
      return {
        success: true,
        message: `Successfully deposited $${amount.toFixed(2)} to agent`
      }
    } else {
      return {
        success: false,
...
export async function withdrawFromAgent(amount: number): Promise<{
  success: boolean
  message: string
  error?: string
}> {
  try {
    ...
    if (success) {
      return {
        success: true,
        message: `Successfully withdrew $${amount.toFixed(2)} from agent`
      }
    } else {
      return {
        success: false,
        message: "Withdrawal failed",
        error: "Insufficient liquidity, please try a smaller amount"
      }
    }
  } catch (error) {
    return {
